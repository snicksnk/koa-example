...
export function processData(raw) {
  ...
  // Sort and Limit
  raw = raw.sort( (a,b) => { return b.score*(b.numLinks+1) - a.score*(a.numLinks+1); } );
  raw = raw.slice(0,200);
  ...
  for ( let [i, value] of raw.entries() ) {
    ...
    nodes[value._id] = {i: i, name: d.name, year: d.year, score: d.score, numLinks: d.numLinks};
  };
  ...
  data.nodes = Object.keys(nodes).map(function (key) { return nodes[key]; });
